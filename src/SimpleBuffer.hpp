...
// frame, and provide access to that value at each station.
//
// Clients Push new elements into the buffer once per frame.
// They can then access elements in the buffer by station index.
//
...
public:
    ...
    T GetValueAtStation(size_t stationIdx) const;
...
template <class T>
T
SrSimpleBuffer<T>::GetValueAtStation(size_t stationIdx) const
{
    return (*this)[stationIdx * _model->GetFramesPerStation()];
}
